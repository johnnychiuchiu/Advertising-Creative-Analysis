...
def find_best_ad(df):
    ...
    df=df.assign(CPC=df.spent/df.link_clicks*1.0) 
    ...
    f = {'score': np.mean, 'impression': np.sum, 'link_clicks': np.sum, 'spent': np.sum }    
...
def find_best_feature(analysis_df):
    ...
    analysis_df=analysis_df.assign(CPC=analysis_df.spent/analysis_df.link_clicks*1.0) 
    ...
    for name in analysis_df:
        if name not in dimension+['impression','link_clicks','spent','CTR','CPC','score']:
            print name
            f = {'score': np.mean, 'impression': np.sum, 'link_clicks': np.sum, 'spent': np.sum }    
...
def feature_importance(analysis_df):
    ...
    feature=feature.drop(['impression','link_clicks','spent','CTR','CPC','score'], axis=1)
...
def feature_importance0(analysis_df):
    import copy
    feature=copy.deepcopy(analysis_df)
